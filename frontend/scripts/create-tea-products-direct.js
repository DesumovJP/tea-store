/**
 * Script to create 20 real tea products directly via Strapi API
 * Run with: node scripts/create-tea-products-direct.js
 * 
...
async function createProducts() {
    ...
    try {
        ...
        if (categories.length === 0) {
            ...
            console.error('   Required categories: Black Tea, Green Tea, White Tea, Oolong Tea, Pu-erh Tea, Herbal Tea');
            process.exit(1);
        }
        
        console.log(`✅ Found ${categories.length} categories:`);
        categories.forEach(cat => console.log(`   - ${cat.name} (${cat.documentId})`));
        console.log('');
        
        // Create products directly via Strapi API
        let successCount = 0;
        let errorCount = 0;
        
        // Helper function to find category (case-insensitive, flexible matching)
        const findCategory = (categoryName) => {
            const normalized = categoryName.toLowerCase().trim();
            return categories.find(c => {
                const catName = c.name.toLowerCase().trim();
                return catName === normalized || 
                       catName === normalized.replace(' tea', '') ||
                       catName === normalized + ' tea' ||
                       catName.replace(' tea', '') === normalized;
            });
        };
        
        for (const product of teaProducts) {
            try {
                // Find category documentId (with flexible matching)
                const category = findCategory(product.categoryName);
                
                if (!category) {
                    console.log(`⚠️  Category "${product.categoryName}" not found for "${product.title}".`);
                ...
                // Create product directly via Strapi API
                const payload = {
                    data: {
                        title: product.title,
                        slug: product.slug,
                ...
                if (!response.ok) {
                    ...
                    console.error(`❌ Failed to create "${product.title}":`, response.status);
                    console.error(`   Error:`, JSON.stringify(error, null, 2));
                    errorCount++;
                } else {
                    const result = await response.json();
                    console.log(`✅ Created: ${product.title} (${product.categoryName}) - $${product.price}`);
                    successCount++;
                }
